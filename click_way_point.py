...
def onclick(event: MouseEvent):
    ...
    if event.xdata is not None and event.ydata is not None:
        ...
        print(f"Waypoint {len(clicked_waypoints)}: (row={int(y)}, column={int(x)})")

        # Display clicked point on the map
        ax.scatter(x, y, color='red', edgecolor='black', s=100, label=f"Waypoint {len(clicked_waypoints)}")
        plt.draw()
