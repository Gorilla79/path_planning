...
def onclick(event: MouseEvent):
    if event.xdata is not None and event.ydata is not None:
        ...
        ax.scatter(x, y, color='red', edgecolor='black', s=100)
        ax.text(x, y, f"{waypoint_number}", color='blue', fontsize=12, ha='center', va='center')
...
if not os.path.exists(grid_file_path):
    ...
    try:
        ...
        plt.title("Grid Map with Adjusted Paths")
        plt.xlabel("X")
        plt.ylabel("Y")
        plt.legend()
        plt.show()
